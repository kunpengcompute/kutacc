...
void dense_mla_h64_kvpacked_kernel(
...
)
{
    ...
    constexpr int max_tiled_blocks = 4;
    constexpr int max_tiled_n = 64 * max_tiled_blocks;

    constexpr int size0 = std::max(
        64 * 576 * sizeof(bfloat16_t) + 64 * max_tiled_n * sizeof(float), 64 * 512 * sizeof(float));
    static_assert(size0 + (64 * 512 + 64 + 64 + 64) * sizeof(float) <= MAX_THREAD_BUFFER_BYTES);
    ...
    float *block_s = (float *)(block_q + 64 * 576);
    bfloat16_t *block_p = (bfloat16_t *)block_s;
    float *block_o = (float *)alloc_buffer(thread_ptr, 64 * 512 * sizeof(float));
    float *row_max = (float *)alloc_buffer(thread_ptr, 64 * sizeof(float));
    float *row_sum = (float *)alloc_buffer(thread_ptr, 64 * sizeof(float));
    float *scale_o = (float *)alloc_buffer(thread_ptr, 64 * sizeof(float));

    int pack_v_stride = ceil_div<int64_t>(seqlen_kv, kv_block_size) * kv_block_size;
    ...
    const bfloat16_t *V = reinterpret_cast<bfloat16_t *>(params.vcache->value().data_ptr());
    ...
    if (likely((q_block_idx + 1) * 64 <= params.num_heads_q)) {
        for (int j = 0; j < 576; j += 32) {
            ...
            const bfloat16_t *q2 = Q + row_offset_q + 32 * 576 + j;
            const bfloat16_t *q3 = Q + row_offset_q + 48 * 576 + j;
            for (int i = 0; i < 16; ++i) {
                svld1_hor_za32(0, i, ptrue, q0);
                svld1_hor_za32(1, i, ptrue, q1);
                svld1_hor_za32(2, i, ptrue, q2);
                svld1_hor_za32(3, i, ptrue, q3);
                q0 += 576;
                q1 += 576;
                q2 += 576;
                q3 += 576;
            ...
            for (int i = 0; i < 16; ++i) {
                svst1_ver_za32(0, i, ptrue, block_q + j * 64 + i * 128);
                svst1_ver_za32(1, i, ptrue, block_q + j * 64 + i * 128 + 32);
                svst1_ver_za32(2, i, ptrue, block_q + j * 64 + i * 128 + 64);
                svst1_ver_za32(3, i, ptrue, block_q + j * 64 + i * 128 + 96);
            }
        }
    } else {
        FLASH_MLA_CHECK(0, "");
    ...
    const int num_tiled_blocks = ceil_div<int64_t>(n_block_max - n_block_min, max_tiled_blocks);
    const int tiled_block_offset = num_tiled_blocks * (q_block_idx / snoop_group_size) /
        ceil_div<int64_t>(params.num_heads_q, 64 * snoop_group_size);
    for (int tiled_block_id = 0; tiled_block_id < num_tiled_blocks; ++tiled_block_id) {
        ...
        const bfloat16_t *nxt_k_ptr = tiled_block_id + 1 < num_tiled_blocks ?
        ...
        for (int offset_n = 0; offset_n < len_n; offset_n += 16) {
            ...
            for (int k = 0; k < 576 / 2; ++k) {
                constexpr int prf_stride = 32 * 6;
                if (likely(p_b != nullptr)) {
                    svprfh(ptrue, p_b, SV_PLDL2KEEP);
                    p_b += 32;
                }
                svbfloat16_t vb = SVLDNT1_BF16(ptrue, b);
        ...
        for (int offset_n = 0; offset_n < 512; offset_n += 16) {
            svzero_za();
            bfloat16_t *a0 = block_p;
            bfloat16_t *a1 = block_p + 16 * tiled_n;
            bfloat16_t *a2 = block_p + 32 * tiled_n;
            bfloat16_t *a3 = block_p + 48 * tiled_n;
            for (int n_tiled_block = 0; n_tiled_block < tiled_blocks; ++n_tiled_block) {
                const bfloat16_t *b =
                    V + block_table[n_block + n_tiled_block] * params.vcache->value().stride(0) + offset_n * 64;
                const bfloat16_t *p_b = b;
                constexpr int step = 16;
                if (offset_n + step < 512) {
                    p_b = V + block_table[n_block + n_tiled_block] * params.vcache->value().stride(0) +
                        (offset_n + step) * 64;
                } else if (tiled_block_id + 1 < num_tiled_blocks) {
                    p_b = V + block_table[n_block_min + (n_block - n_block_min + max_tiled_blocks +
                        n_tiled_block) % (n_block_max - n_block_min)] * params.vcache->value().stride(0);
                }
                for (int k = 0; k < 64 / 2; ++k) {
                    ...
                    svmopa_za32_bf16_m(1, ptrue, ptrue, va1, vb);
                    svbfloat16_t va2 = svld1(ptrue, a2);
                    svmopa_za32_bf16_m(2, ptrue, ptrue, va2, vb);
                    svbfloat16_t va3 = svld1(ptrue, a3);
                    svmopa_za32_bf16_m(3, ptrue, ptrue, va3, vb);
                    a0 += 32;
                    a1 += 32;
                    a2 += 32;
            ...
            float *c0 = block_o + offset_n * 64;
            float *c1 = block_o + offset_n * 64 + 16 * 16;
            float *c2 = block_o + offset_n * 64 + 32 * 16;
            float *c3 = block_o + offset_n * 64 + 48 * 16;
            for (int t = 0; t < 16; ++t) {
                ...
                if (likely(!is_first_step)) {
                    ...
                    vc0 = svadd_m(ptrue, vc0, svmul_m(ptrue, vd0, scale_o[t]));
                    vc1 = svadd_m(ptrue, vc1, svmul_m(ptrue, vd1, scale_o[t + 16]));
                    vc2 = svadd_m(ptrue, vc2, svmul_m(ptrue, vd2, scale_o[t + 32]));
                    vc3 = svadd_m(ptrue, vc3, svmul_m(ptrue, vd3, scale_o[t + 48]));
                }
    ...
    for (int i = 0; i < q_block_size; ++i) {
        for (int j = 0; j < 512; j += 16) {
            svst1(ptrue, out_o + i * 512 + j, svld1(ptrue, block_o + j * q_block_size + i * 16));
        }
    }
    MATRIX_OFF();
