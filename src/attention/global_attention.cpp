...
namespace kutacc {
void global_attention_kernel(Tensor &q_avg, Tensor &q, Tensor &k, Tensor &v, int64_t batch, 
    int64_t seq_len, int64_t nchannels, int64_t nheads, int64_t head_size, Tensor &gate, 
    Tensor &q_data, Tensor &q_mask, const Tensor &query_w, const Tensor &key_w, 
    const Tensor &value_w, const Tensor &gating_w, const Tensor& gating_b, const Tensor &output_w,
    const Tensor & output_b, Tensor &out)
{
    KUTACC_CHECK(batch > 0 && seq_len > 0 && nchannels > 0 && nheads > 0 && head_size > 0 &&
        batch <= INT32_MAX / seq_len && seq_len <= INT32_MAX && nchannels <= INT32_MAX && head_size <= INT32_MAX &&
        nheads <= INT64_MAX / seq_len && nheads <= INT64_MAX / head_size, "input param <= 0 or overflow");
    ...
    addmm(kutacc::to_bf16(1),
        Tensor((__bf16 *)q_avg.data_ptr(), {batch, nchannels}, {nchannels, 1}, 2, kBF16),
        Tensor((__bf16 *)query_w.data_ptr(), {nchannels, nchannels}, {1, nchannels}, 2, kBF16),
        kutacc::to_bf16(0),
        Tensor((__bf16 *)q.data_ptr(), {batch, nchannels}, {nchannels, 1}, 2, kBF16),
        BlasExtendParams{.prepack_a = false, 
            .prepack_b = true});
    addmm(kutacc::to_bf16(1),
        Tensor((__bf16 *)input.data_ptr(), {batch * seq_len, nchannels}, {nchannels, 1}, 2, kBF16),
        Tensor((__bf16 *)key_w.data_ptr(), {nchannels, head_size}, {1, nchannels}, 2, kBF16),
        kutacc::to_bf16(0),
        Tensor((__bf16 *)k.data_ptr(), {batch * seq_len, head_size}, {head_size, 1}, 2, kBF16),
        BlasExtendParams{.prepack_a = false, 
            .prepack_b = true});
    addmm(kutacc::to_bf16(1),
        Tensor((__bf16 *)value_w.data_ptr(), {head_size, nchannels}, {nchannels, 1}, 2, kBF16),
    ...
    addmm(kutacc::to_bf16(1),
        Tensor((__bf16 *)input.data_ptr(), {batch * seq_len, nchannels}, {nchannels, 1}, 2, kBF16),
        Tensor((__bf16 *)gating_w.data_ptr(), {nchannels, nchannels}, {1, nchannels}, 2, kBF16),
        kutacc::to_bf16(0),
    ...
    parallel_for(0, batch, 1, [&](int64_t start, int64_t end) {
        ...
        for (int64_t bi = start; bi < end; bi++) {
            ...
            addmm(kutacc::to_bf16(1),
                Tensor((__bf16 *)q.data_ptr() + bi * q.strides()[0], {nheads, head_size}, 
                    {head_size, 1}, 2, kBF16),
                Tensor((__bf16 *)k.data_ptr() + bi * k.strides()[0], {head_size, seq_len}, 
                    {1, head_size}, 2, kBF16),
                kutacc::to_bf16(0), logits,
            ...
            for (int64_t hi = 0; hi < nheads; hi++) {
                ...
                for (int64_t si = 0; si < seq_len; si += vl) {
                    ...
                    auto bias_values = svmla_x(pg, svdup_f32(-1e9f), 
                    kutacc::svld1<float, __bf16>(pg, &mask_data[si]), 1e9f);
            ...
            addmm(kutacc::to_bf16(1), logits,
                Tensor((__bf16 *)v.data_ptr() + bi * v.strides()[1], {seq_len, head_size}, 
                    {1, v.strides()[0]}, 2, kBF16),
                kutacc::to_bf16(0),
                Tensor(weighted_avg.get(), {nheads, head_size}, {head_size, 1}, 2, kBF16),
                BlasExtendParams{
                    .num_threads = 1
                });
            if (nchannels == 64) {
    ...
    addmm(kutacc::to_bf16(1),
        Tensor((__bf16 *)gate.data_ptr(), {batch * seq_len, nchannels}, {gate.strides()[1], 1}, 2, kBF16),
        Tensor((__bf16 *)output_w.data_ptr(), {nchannels, nchannels}, {1, nchannels}, 2, kBF16),
        kutacc::to_bf16(0),
    ...
    kutacc_tensor_h q_mask, kutacc_af2_attention_weights_t *weight_ptr, kutacc_tensor_h out)
{
    KUTACC_CHECK(q_based_ptr != nullptr && q_data != nullptr && q_mask != nullptr && weight_ptr != nullptr &&
        weight_ptr != nullptr && out != nullptr && weight_ptr->nchannels == 64, "kutacc_af2_gating_attention: input args nullptr error or invalid int value error");
    if (kutacc::kutacc_check_err_set == true) {
        return;
    }
    ...
    kutacc::global_attention_kernel(*kutacc::convertKutaccTensor(q_avg), *kutacc::convertKutaccTensor(q), *kutacc::convertKutaccTensor(k),
        *kutacc::convertKutaccTensor(v), batch, seq_len, nchannels, nheads, head_size, *kutacc::convertKutaccTensor(gate),*kutacc::convertKutaccTensor(q_data),
        *kutacc::convertKutaccTensor(q_mask), *kutacc::convertKutaccTensor(query_w), *kutacc::convertKutaccTensor(key_w), *kutacc::convertKutaccTensor(value_w),
        *kutacc::convertKutaccTensor(gating_w), *kutacc::convertKutaccTensor(gating_b), *kutacc::convertKutaccTensor(output_w), 
