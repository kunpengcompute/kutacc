...
namespace kutacc {
void kutacc_af2_invariant_point_kernel(Tensor q, Tensor k, Tensor v, Tensor q_pts, Tensor k_pts, Tensor v_pts, 
    Tensor b, Tensor a, Tensor head_weights, Tensor weights_head_weights, Tensor o, Tensor o_pt, Tensor o_pt_norm, Tensor o_pair,
    Tensor z, Tensor rigid_rot_mats, Tensor rigid_trans, Tensor mask, Tensor linear_b_w, Tensor linear_b_b,
    int64_t n_res, int64_t c_z, int64_t c_hidden, int64_t no_heads, int64_t no_qk_points, int64_t no_v_points)
{
    KUTACC_CHECK(n_res > 0 && c_z > 0 && c_hidden > 0 && no_heads > 0 && no_qk_points > 0 && no_v_points > 0 &&
        n_res <= INT64_MAX / no_heads, "input param <= 0 or overflow");
    if (kutacc::kutacc_check_err_set == true) {
        return;
    }
    addmm(to_bf16(1),
        Tensor((__bf16 *)linear_b_w.data_ptr(), {no_heads, c_z}, {linear_b_w.strides()[0], 1}, 2, kBF16),
        Tensor((__bf16 *)z.data_ptr(), {c_z, n_res * n_res}, {1, z.strides()[1]}, 2, kBF16),
        to_bf16(0),
    ...
    parallel_for(0, no_heads * n_res, 1, [&](int64_t start, int64_t end) {
        ...
        kutacc::collapse_for(start, end, no_heads, n_res, [&](int64_t hi, int64_t qi){
            for (int64_t ki = 0; ki < n_res; ki++) {
                auto q_values = svld1<float, __bf16>(pg_pt_att,
                    (__bf16 *)q_pts.data_ptr() + qi * q_pts.strides()[0] + hi * q_pts.strides()[1]);
                auto k_values = svld1<float, __bf16>(pg_pt_att,
                    (__bf16 *)k_pts.data_ptr() + ki * k_pts.strides()[0] + hi * k_pts.strides()[1]);
            ...
            for (int64_t ki = 0; ki < n_res; ki += vl) {
                ...
                auto values = svld1<float, __bf16>(pg,
                    (__bf16 *)a.data_ptr() + hi * a.strides()[0] + qi * a.strides()[1] + ki);
                {
                    auto b_values = svld1<float, __bf16>(pg,
                        (__bf16 *)b.data_ptr() + hi * b.strides()[0] + qi * b.strides()[1] + ki);
    ...
    parallel_for(0, n_res, 1, [&](int64_t start, int64_t end) {
        for (int64_t ri = start; ri < end; ri ++) {
            addmm(to_bf16(1),
                ...
                to_bf16(0),
                Tensor((__bf16 *)o_pair.data_ptr() + ri * o_pair.strides()[0], {no_heads, c_z}, {o_pair.strides()[1], 1}, 2, kBF16),
...
{
    KUTACC_CHECK(ipa_s_ptrs != nullptr && ipa_o_ptrs != nullptr && z != nullptr && rigid_rot_mats != nullptr && rigid_trans != nullptr && mask != nullptr && ipa_weight_ptrs != nullptr,
    ...
    kutacc::kutacc_af2_invariant_point_kernel(*kutacc::convertKutaccTensor(q), *kutacc::convertKutaccTensor(k), *kutacc::convertKutaccTensor(v), *kutacc::convertKutaccTensor(q_pts), *kutacc::convertKutaccTensor(k_pts), *kutacc::convertKutaccTensor(v_pts), 
        *kutacc::convertKutaccTensor(b), *kutacc::convertKutaccTensor(a), *kutacc::convertKutaccTensor(head_weights), *kutacc::convertKutaccTensor(weights_head_weights), *kutacc::convertKutaccTensor(o), *kutacc::convertKutaccTensor(o_pt), *kutacc::convertKutaccTensor(o_pt_norm), *kutacc::convertKutaccTensor(o_pair),
        *kutacc::convertKutaccTensor(z), *kutacc::convertKutaccTensor(rigid_rot_mats), *kutacc::convertKutaccTensor(rigid_trans), *kutacc::convertKutaccTensor(mask), *kutacc::convertKutaccTensor(linear_b_w), *kutacc::convertKutaccTensor(linear_b_b),
