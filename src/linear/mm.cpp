...
namespace kutacc {
    namespace internal {
        ...
        {
            KUTACC_CHECK(a.sizes()[1] == b.sizes()[0] && c.sizes()[0] == a.sizes()[0] && c.sizes()[1] == b.sizes()[1], "shape invalid, a: [", 
                        a.sizes()[0], ", ", a.sizes()[1], "], b: [", b.sizes()[0], ", ", b.sizes()[1], "], c: [", c.sizes()[0], ", ", c.sizes()[1],
                        "]");
            KUTACC_CHECK(a.strides()[0] == 1 || a.strides()[1] == 1, "a strides invalid: [", a.strides()[0], ", ", a.strides()[1], "]");
            KUTACC_CHECK(b.strides()[0] == 1 || b.strides()[1] == 1, "b strides invalid: [", b.strides()[0], ", ", b.strides()[1], "]");
            KUTACC_CHECK(c.strides()[1] == 1, "c strides invalid: [", c.strides()[0], ", ", c.strides()[1], "]");
            KUTACC_CHECK(a.dim() == DIM_2 && b.dim() == DIM_2 && c.dim() == DIM_2, "dim invalid, a: [", 
                        a.dim(), "], b: [", b.dim(), "], c: [", c.dim(), "]");
        }
}   // namespace internel
 ...
 */
    ...
    inline void addmm_template(scalar_out_t alpha, const Tensor &a, 
                               const Tensor &b, scalar_out_t beta, const Tensor &c, 
                               kutacc::BlasExtendParams param= {})
    {
        ...
        kutacc::gemm(transb, transa, n, m, k, alpha, static_cast<const scalar_t*>(b.data_ptr()), 
            (int)ldb, static_cast<const scalar_t*>(a.data_ptr()), (int)lda, beta, 
            static_cast<scalar_t*>(c.data_ptr()), (int)c.strides()[0], param);
    }
    ...
    void addmm(__bf16 alpha, const kutacc::Tensor &a,
                ...
                kutacc::BlasExtendParams param)
    {
        KUTACC_CHECK(a.data_ptr() != nullptr && b.data_ptr() != nullptr && c.data_ptr() != nullptr, 
                     "addmm input invalid");
...
*/
kutacc_export void kutacc_af2_linear_weight_prepack(const __bf16 *weight, __bf16 *result, int64_t n, int64_t k, int64_t ldb, int64_t num_threads)
{
    if (num_threads != 1) {
