...
void test_batch_bf16_s8_packed_gemm_bf16(std::vector<int64_t> &cas)
{
    int64_t bs = cas[0];
    int64_t m = cas[1];
    int64_t n = cas[2];
    ...
    for (auto rowQuant : {0, 1}) {
        ...
        kutacc::batch_bf16_gemm_pack(bs, m, k, m * k, k, a.get(), pa.get(), 2);
        kutacc::batch_bf16_gemm_pack(bs, n, k, n * k, k, b.get(), pb.get(), 1);
        kutacc::batch_bf16_s8_packed_gemm_bf16(
            bs, m, n, k, m * n, n, pa.get(), pb.get(), c.get(), quantArgs[0], quantArgs[1]);
        for (int64_t i = 0; i < bs * m; i++) {
            for (int64_t j = 0; j < n; j++) {
...
int main(int argc, char **argv)
{
    kutacc::global_parallel_launch([&] {
        ...
        for (auto cas : cases) {
            test_batch_bf16_s8_packed_gemm_bf16(cas);
        }
    });
}
