...
# Get the path of installation
if [ -n "$3" ]; then
    install_path="$3"
else
    install_path="numa_duplication"
fi
...
do
    (
        path_numa="$install_path/numa_$i"
        path_numa_lib="$install_path/numa_$i/lib"
        mkdir -p $path_numa
        mkdir -p $path_numa_lib
        cp $app_path $path_numa
        replace_needed "$path_numa/$app_name" "$path_numa_lib"
