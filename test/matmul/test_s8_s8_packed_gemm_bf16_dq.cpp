...
void test_s8_s8_packed_gemm_bf16_dq(std::vector<int64_t> &cas)
{
    int64_t m = cas[0];
    int64_t n = cas[1];
    int64_t k = cas[2];
    ...
    kutacc::s8_gemm_pack(m, k, std::get<0>(t), std::get<2>(t), a.get(), pack_a.get());
    kutacc::s8_gemm_pack(n, k, std::get<1>(t), std::get<2>(t), b.get(), pack_b.get());
    kutacc::s8_s8_packed_gemm_bf16_dq(
        m, n, k, t, pack_a.get(), pack_b.get(), quantArgs[1], quantArgs[0], c.get(), tmpc.get());
    if (k == std::get<2>(t)) {
        for (int64_t i = 0; i < m; i++) {
            for (int64_t j = 0; j < n; j++) {
                float cans = c[i * n + j];
        ...
        for (int64_t i = 0; i < m; i++) {
            for (int64_t j = 0; j < n; j++) {
                int64_t idx = i * n + j;
                float a = to_float(c[idx]);
                float b = to_float(expect[idx]);
                dot += (double)a * b;
                nc += (double)a * a;
                ne += (double)b * b;
            }
        ...
        double cos_diff = 1.0;
        if (sqrt(nc) > 1e-8 && sqrt(ne) > 1e-8) {
            cos_diff = 1.0 - dot / (sqrt(nc) * sqrt(ne));
        } else if (nc < 1e-8 && ne < 1e-8) {
            cos_diff = 0.0;
        }
...
int main(int argc, char **argv)
{
    kutacc::global_parallel_launch([&] {
        ...
        for (auto cas : cases) {
            test_s8_s8_packed_gemm_bf16_dq(cas);
        }
    });
}
