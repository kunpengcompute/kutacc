...
namespace kutacc {
void af2_transpose_kernel(Tensor &data, Tensor &out)
{
    int64_t m = std::max(data.sizes()[0], out.sizes()[0]);
    int64_t n = std::max(data.sizes()[1], out.sizes()[1]);
    ...
    int64_t subblock_m = buffer_size / (world_size * block_n * len);
    KUTACC_CHECK(subblock_m > 0, buffer_size, " ", world_size, " ", block_n, " ", len);
    for (int64_t block_mi_start = 0; block_mi_start < block_m; block_mi_start += subblock_m) {
        ...
        parallel_for(0, par_size, 1, [&](int64_t start, int64_t end) {
            collapse_for(start, end, world_size, size_m, block_n, [&](int64_t ri, int64_t sbmi, int64_t bni) {
        ...
        parallel_for(0, par_size, 1, [&](int64_t start, int64_t end) {
            collapse_for(start, end, world_size, size_m, block_n, [&](int64_t ri, int64_t sbmi, int64_t bni) {
                int64_t bmi = sbmi + block_mi_start;
                if (bmi < m - ri * block_m && bni < n - rank * block_n) {
                    if (ri != rank) {
                        void *remote_buffer;
...
void kutacc_af2_transpose(kutacc_tensor_h data, kutacc_tensor_h out)
{
    ...
    af2_transpose_kernel(*kutacc::convertKutaccTensor(data), *kutacc::convertKutaccTensor(out));
}
