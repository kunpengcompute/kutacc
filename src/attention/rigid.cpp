        ...
        if (pts.dim() == 2) {
            collapse_for(start, end, pts.sizes()[0], [&](int64_t i0) {
                ...
                if (trans != nullptr) {
                    ...
                    trans_data =
                        (float *)trans_t.data_ptr() + trans_t.strides()[0] * (trans_t.sizes()[0] != 1 ? i0 : 0);
                }
            ...
            collapse_for(start, end, pts.sizes()[0], pts.sizes()[1], [&](int64_t i0, int64_t i1) {
                ...
                auto out_data = (scalar_t *)out.data_ptr() + out.strides()[0] * i0 + out.strides()[1] * i1;
                float *trans_data = nullptr;
                if (trans != nullptr) {
                    auto trans_t = *convertKutaccTensor(trans);
                    trans_data = (float *)trans_t.data_ptr() +
                                 trans_t.strides()[0] * (trans_t.sizes()[0] != 1 ? i0 : 0) +
            ...
            collapse_for(start, end, pts.sizes()[0], pts.sizes()[1], pts.sizes()[2],
                         [&](int64_t i0, int64_t i1, int64_t i2) {
                             auto pts_data = (scalar_t *)pts.data_ptr() + pts.strides()[0] * i0 +
                                             pts.strides()[1] * i1 + pts.strides()[2] * i2;
                             auto rot_mats_data = (float *)rot_mats.data_ptr() +
                                                  rot_mats.strides()[0] * (rot_mats.sizes()[0] != 1 ? i0 : 0) +
                                                  rot_mats.strides()[1] * (rot_mats.sizes()[1] != 1 ? i1 : 0) +
...
void kutacc_af2_rigid_rot_vec_mul(kutacc_tensor_h pts, kutacc_tensor_h rot_mats, kutacc_tensor_h out,
                                  kutacc_tensor_h trans)
{
    KUTACC_CHECK(out != nullptr && pts != nullptr && rot_mats != nullptr,
                 "kutacc_af2_rigid_rot_vec_mul: input args nullptr error");
    if (kutacc::kutacc_check_err_set == true) {
        return;
    }
    if ((*kutacc::convertKutaccTensor(pts)).dtype() == kutacc::kBF16) {
        kutacc::rigid_rot_vec_mul<__bf16>(*kutacc::convertKutaccTensor(pts), *kutacc::convertKutaccTensor(rot_mats),
                                          *kutacc::convertKutaccTensor(out), trans);
    }
}

void kutacc_af2_rigid_rot_matmul(kutacc_tensor_h a, kutacc_tensor_h b, kutacc_tensor_h out)
{
    KUTACC_CHECK(a != nullptr && b != nullptr && out != nullptr,
                 "kutacc_af2_rigid_rot_matmul: input args nullptr error");
    if (kutacc::kutacc_check_err_set == true) {
        return;
    }
    kutacc::rigid_rot_matmul(*kutacc::convertKutaccTensor(a), *kutacc::convertKutaccTensor(b),
                             *kutacc::convertKutaccTensor(out));
}
