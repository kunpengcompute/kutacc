...
constexpr int64_t ON_PACKAGE_MEMORY_SIZE =
	(3'000'000'000 + ON_PACKAGE_MEMORY_ALIGNMENT - 1) / ON_PACKAGE_MEMORY_ALIGNMENT * ON_PACKAGE_MEMORY_ALIGNMENT;
// now use kupl_barrier set 50
constexpr int64_t SHM_SIZE =
    (60'000'000 + ON_PACKAGE_MEMORY_ALIGNMENT - 1) / ON_PACKAGE_MEMORY_ALIGNMENT * ON_PACKAGE_MEMORY_ALIGNMENT;
...
void init_memory_pool()
{
    ...
    kupl_shm_comm_create(intra_socket_size, intra_socket_rank, global_rank, oob_cbs_h,
        (void *)(&intra_socket_comm), &kupl_intra_socket_comm);
    ...
    if (success == -1) {
        printf("mbind failed! errno=%d, desc=%s\n", errno, strerror(errno));
        exit(1);
    }
    KUTACC_CHECK(success == 0, "node ", params.world_rank / params.local_size, ", numa ", params.local_rank);
