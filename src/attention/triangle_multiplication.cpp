 ...
 * [out] proj_act
 */
void calc_proj_act(Tensor &proj_act, Tensor &gate, Tensor &input_act, Tensor &mask, const Tensor &proj_w,
                   const Tensor &proj_b, const Tensor &gate_w, const Tensor &gate_b, int64_t n_res,
                   int64_t n_res_gather, int64_t c_o, int64_t c_i, bool input_prepack)
{
    KUTACC_CHECK(n_res > 0 && n_res_gather > 0 && c_o > 0 && c_i > 0 && n_res <= INT64_MAX / n_res_gather &&
                     n_res <= INT64_MAX / c_i && c_o <= INT32_MAX,
                 "input param <= 0 or overflow");
    if (kutacc::kutacc_check_err_set == true) {
        return;
    }
    // 矩阵乘对结果转置
    addmm(to_bf16(1), Tensor((__bf16 *)proj_w.data_ptr(), {c_i, c_o}, {c_o, 1}, 2, kBF16),
          Tensor((__bf16 *)input_act.data_ptr(), {c_o, n_res * n_res_gather}, {1, c_o}, 2, kBF16), to_bf16(0),
          Tensor((__bf16 *)proj_act.data_ptr(), {c_i, n_res * n_res_gather}, {proj_act.strides()[0], 1}, 2, kBF16),
          BlasExtendParams{.prepack_a = true, .prepack_b = input_prepack, .col_bias = true, .bias = proj_b.data_ptr()});
    // 矩阵乘对结果转置
    addmm(to_bf16(1), Tensor((__bf16 *)gate_w.data_ptr(), {c_i, c_o}, {c_o, 1}, 2, kBF16),
          Tensor((__bf16 *)input_act.data_ptr(), {c_o, n_res * n_res_gather}, {1, c_o}, 2, kBF16), to_bf16(0),
          Tensor((__bf16 *)gate.data_ptr(), {c_i, n_res * n_res_gather}, {gate.strides()[0], 1}, 2, kBF16),
          BlasExtendParams{.prepack_a = true, .prepack_b = input_prepack, .col_bias = true, .bias = gate_b.data_ptr()});
    ...
    parallel_for(0, c_i * n_res, 1, [&](int64_t start, int64_t end) {
        ...
        for ([[maybe_unused]] int64_t j = start; j < end; j++) {
            ...
            auto gate_data = (__bf16 *)gate.data_ptr() + ci * gate.strides()[0] + bi * gate.strides()[1];
 ...
 * [out] center_act
 */
void equation(Tensor &center_act, Tensor &left_proj_act, Tensor &right_proj_act, int64_t n_res_gather, bool is_incoming)
{
    parallel_for(0, left_proj_act.sizes()[0], 1, [&](int64_t start, int64_t end) {
        for (int64_t i = start; i < end; i++) {
            if (is_incoming == 0) {
                addmm(to_bf16(1),
                      Tensor((__bf16 *)left_proj_act.data_ptr() + i * left_proj_act.strides()[0],
                             {n_res_gather, n_res_gather}, {left_proj_act.strides()[1], 1}, 2, kBF16),
                      Tensor((__bf16 *)right_proj_act.data_ptr() + i * right_proj_act.strides()[0],
                             {n_res_gather, n_res_gather}, {1, right_proj_act.strides()[1]}, 2, kBF16),
                      to_bf16(0),
                      Tensor((__bf16 *)center_act.data_ptr() + i * center_act.strides()[0],
 ...
 * [out] gate, out
 */
void gate_and_out_linear(Tensor &gate, Tensor &out, Tensor &input_act, Tensor &center_act, const Tensor &gating_w,
                         const Tensor &gating_b, const Tensor &output_proj_w, const Tensor &output_proj_b,
                         int64_t n_res, int64_t n_res_gather, int64_t c_o, int64_t c_i, bool input_prepack)
{
    KUTACC_CHECK(n_res > 0 && n_res_gather > 0 && c_o > 0 && c_i > 0 && n_res <= INT64_MAX / n_res_gather &&
                     c_o <= INT32_MAX && c_i <= INT32_MAX,
                 "input param <= 0 or overflow");
    if (kutacc::kutacc_check_err_set == true) {
        return;
    }
    addmm(
        to_bf16(1), Tensor((__bf16 *)input_act.data_ptr(), {n_res * n_res_gather, c_o}, {c_o, 1}, 2, kBF16),
        Tensor((__bf16 *)gating_w.data_ptr(), {c_o, c_o}, {1, c_o}, 2, kBF16), to_bf16(0),
        Tensor((__bf16 *)gate.data_ptr(), {n_res * n_res_gather, c_o}, {gate.strides()[1], 1}, 2, kBF16),
        BlasExtendParams{.prepack_a = input_prepack, .prepack_b = true, .row_bias = true, .bias = gating_b.data_ptr()});

    addmm(to_bf16(1),
          Tensor((__bf16 *)center_act.data_ptr(), {n_res * n_res_gather, c_i}, {center_act.strides()[1], 1}, 2, kBF16),
          Tensor((__bf16 *)output_proj_w.data_ptr(), {c_i, c_o}, {1, c_i}, 2, kBF16), to_bf16(0),
          Tensor((__bf16 *)out.data_ptr(), {n_res * n_res_gather, c_o}, {out.strides()[1], 1}, 2, kBF16),
                                                                ...
                                                                bool input_prepack)
{
    ...
    kutacc::calc_proj_act(*kutacc::convertKutaccTensor(proj_act), *kutacc::convertKutaccTensor(gate),
                          *kutacc::convertKutaccTensor(input_act), *kutacc::convertKutaccTensor(mask),
                          *kutacc::convertKutaccTensor(proj_w), *kutacc::convertKutaccTensor(proj_b),
                          *kutacc::convertKutaccTensor(gate_w), *kutacc::convertKutaccTensor(gate_b), n_res,
                          n_res_gather, c_o, c_i, input_prepack);
}
                                                               ...
                                                               bool is_incoming)
{
    ...
    KUTACC_CHECK(n_res_gather > 0, "kutacc_af2_triangle_multiplication_equation: input args int values error, values "
                                   "are less than or equal to zero\n");
    if (kutacc::kutacc_check_err_set == true) {
        return;
    }
    kutacc::equation(*kutacc::convertKutaccTensor(center_act), *kutacc::convertKutaccTensor(left_proj_act),
                     *kutacc::convertKutaccTensor(right_proj_act), n_res_gather, is_incoming);
}
...
{
    KUTACC_CHECK(gate != nullptr && out != nullptr && tm_acts_ptr != nullptr && center_act != nullptr &&
                     tm_weights_ptr != nullptr,
    ...
    kutacc::gate_and_out_linear(
        *kutacc::convertKutaccTensor(gate), *kutacc::convertKutaccTensor(out), *kutacc::convertKutaccTensor(input_act),
        *kutacc::convertKutaccTensor(center_act), *kutacc::convertKutaccTensor(gating_w),
        *kutacc::convertKutaccTensor(gating_b), *kutacc::convertKutaccTensor(output_proj_w),
        *kutacc::convertKutaccTensor(output_proj_b), n_res, n_res_gather, c_o, c_i, input_prepack);
}
...
{
    KUTACC_CHECK(out != nullptr && gate != nullptr,
                 "kutacc_af2_triangle_multiplication_last: input args nullptr error\n");
    KUTACC_CHECK(n_res > 0 && n_res_gather > 0 && c_o > 0 && n_res <= INT64_MAX / n_res_gather,
                 "kutacc_af2_triangle_multiplication_last: input args int values error, values are less than or equal "
                 "to zero, or overflow\n");
    if (kutacc::kutacc_check_err_set == true) {
        return;
    }
