...
void s8_gemm_depack(int64_t m, int64_t n, int64_t tm, int64_t tn, int8_t *src, int8_t *dst)
{
...
void bf16_gemm_depack(int64_t m, int64_t n, int64_t tm, int64_t tn, bfloat16_t *src, bfloat16_t *dst)
{
    for (int64_t i = 0; i < m; i += tm) {
        for (int64_t j = 0; j < n; j += tn) {
            int64_t offset = i * n + j * tm;
...
void test_batched_bf16_gemm_pack(std::vector<int64_t> &cas)
{
    int64_t bs = cas[0];
    int64_t m = cas[1];
    int64_t n = cas[2];
    ...
    kutacc::batch_bf16_gemm_pack(bs, m, k, m * k, k, a.get(), pa.get(), 1);
    s8_gemm_depack(bs * m, k, bs * m, k, pa.get(), tmpa.get());
    for (int i = 0; i < bs * m; i++) {
        for (int j = 0; j < k; j++) {
            if (tmpa[i * k + j] != a[i * k + j]) {
                fprintf(stderr, "diff pos in batch_bf16_gemm_pack A (%d, %d)\n", i, j);
                exit(1);
            }
        }
    }

    kutacc::batch_bf16_gemm_pack(bs, n, k, n * k, k, b.get(), pb.get(), 2);
    bf16_gemm_depack(bs * n, k, bs * n, k, pb.get(), tmpb.get());
    for (int i = 0; i < bs * n; i++) {
        for (int j = 0; j < k; j++) {
            if (tmpb[i * k + j] != b[i * k + j]) {
                fprintf(stderr, "diff pos in batch_bf16_gemm_pack B (%d, %d)\n", i, j);
                exit(1);
            }
        }
    }
...
int main(int argc, char **argv)
{
    kutacc::global_parallel_launch([&] {
        ...
        for (auto cas : cases) {
            test_batched_bf16_gemm_pack(cas);
        }
    });
}
