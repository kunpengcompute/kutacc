...
void test_bf16_gemm_pack(std::vector<int64_t> &cas)
{
    int64_t m = cas[0];
    int64_t n = cas[1];
    int64_t tm = cas[2];
    ...
    // bf16_gemm_pack cas
    kutacc::bf16_gemm_pack(m, n, tm, tn, act.get(), out.get());
    bf16_gemm_depack(m, n, tm, tn, out.get(), tmp.get());
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
            if (tmp[i * n + j] != act[i * n + j]) {
                fprintf(stderr, "diff pos in bf16_gemm_pack (%d, %d)\n", i, j);
                exit(1);
            }
        }
    }
