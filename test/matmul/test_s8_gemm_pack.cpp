...
void s8_gemm_vertical_shift(int64_t m, int64_t n, int64_t shift_size, int8_t *src, int8_t *dst)
{
    if (shift_size < 0) {
...
void test_s8_gemm_pack(std::vector<int64_t> &cas)
{
    int64_t m = cas[0];
    int64_t n = cas[1];
    int64_t tm = cas[2];
    ...
    // normal cas
    kutacc::s8_gemm_pack(m, n, tm, tn, act.get(), out.get());
    s8_gemm_depack(m, n, tm, tn, out.get(), tmp.get());

    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
            if (tmp[i * n + j] != act[i * n + j]) {
                fprintf(stderr, "diff pos in s8_gemm_pack (%d, %d)\n", i, j);
                exit(1);
            }
        }
    }
    ...
    // with_idx cas
    kutacc::s8_gemm_pack(m, n, tm, tn, act.get(), out.get(), n, true, idx.get());
    s8_gemm_depack(m, n, tm, tn, out.get(), tmp.get());
    s8_gemm_vertical_shift(m, n, shift_size, tmp.get(), out.get());
