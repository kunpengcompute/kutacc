...
)
{
    ...
    static_assert(head_dim == 576 || head_dim == 512);
    static_assert(head_dim_v == 512);

    constexpr int max_tile_len = 128;
    ...
    constexpr auto micro_block_k_bytes = 32 * head_dim * sizeof(bfloat16_t);
    constexpr auto softmax_bytes = q_block_size * sizeof(float);
    constexpr auto kv_token_addr_bytes = (max_tile_len + max_tile_len / 2 + 16) * sizeof(bfloat16_t *);
    ...
    for (int64_t kv_idx = 0; kv_idx < total_kv_len; kv_idx += max_tile_len) {
        ...
        const auto ceil_kv_len = ceil(kv_len, 32);
        const auto pad_kv_len = ceil_kv_len + ceil_kv_len / 2 + 16;
        ...
        auto prf_kv_token_addr = kv_token_addr + ceil_kv_len / 2 + 16;
        int prf_cnt = 0;
        auto prf_l2 = [&] {
            svprfh(ptrue, prf_kv_token_addr[prf_cnt / (head_dim / 32)] + prf_cnt % (head_dim / 32) * 32, SV_PLDL2KEEP);
            prf_cnt++;
        };
        for (int offset_n = 0; offset_n < kv_len; offset_n += 32) {
            auto cur_kv_token_addr = kv_token_addr + offset_n;
            for (int j = 0; j < head_dim; j += 32) {
                for (int i0 = 0; i0 < 32; i0 += 16) {
                    for (int i = 0; i < 16; ++i) {
                        svld1_hor_za32(0, i, ptrue, cur_kv_token_addr[i0 + i] + j);
                    }
                    auto k1 = micro_block_k + j * 32 + i0 * 2;
                    for (int i = 0; i < 16; ++i) {
            ...
            for (int k = 0; k < head_dim / 2; ++k) {
                ...
                svprfh(ptrue, a + prf_stride, SV_PLDL1STRM);
                svmopa_za32_bf16_m(0, ptrue, ptrue, va0, vb0);
                svbfloat16_t vb1 = SVLDNT1_INDEX_BF16(ptrue, b, 32);
                svprfh(ptrue, a + 32 + prf_stride, SV_PLDL1STRM);
                svmopa_za32_bf16_m(1, ptrue, ptrue, va0, vb1);
                svbfloat16_t va1 = svld1(ptrue, a + 32);
                svprfh(ptrue, b + prf_stride, SV_PLDL1STRM);
                svmopa_za32_bf16_m(2, ptrue, ptrue, va1, vb0);
                svprfh(ptrue, b + 32 + prf_stride, SV_PLDL1STRM);
            ...
            float *c1 = block_s + offset_n * 16 + 16 * 16;
            float *c2 = block_s + offset_n * 16 + 16 * ceil_kv_len;
            float *c3 = block_s + offset_n * 16 + 16 * ceil_kv_len + 16 * 16;
            for (int i = 0; i < 16; ++i) {
        ...
        for (int offset_n = 0; offset_n < head_dim_v; offset_n += 32) {
            ...
            float *c0 = block_o + offset_n * 32;
            float *c1 = block_o + offset_n * 32 + 16;
            float *c2 = block_o + offset_n * 32 + 16 * 32;
            float *c3 = block_o + offset_n * 32 + 16 * 32 + 16;
            for (int t = 0; t < 16; ++t) {
                ...
                if (likely(!is_first_step)) {
                    ...
                    vc1 = svadd_m(ptrue, vc1, svmul_m(ptrue, vd1, scale_o[t]));
                    vc2 = svadd_m(ptrue, vc2, svmul_m(ptrue, vd2, scale_o[16 + t]));
                    vc3 = svadd_m(ptrue, vc3, svmul_m(ptrue, vd3, scale_o[16 + t]));
                }
