...
struct alignas(64) Tensor {
    ...
    kutacc::Tensor<dtype, dim> to_kutacc_tensor() const
    {
        return kutacc::Tensor<dtype, dim>(data_ptr<dtype>(), sizes, strides);
    }
    template <typename dtype, int64_t dim>
    std::optional<kutacc::Tensor<dtype, dim>> to_optional_kutacc_tensor() const
    {
        return ptr == nullptr ? std::optional<kutacc::Tensor<dtype, dim>>(std::nullopt)
            : kutacc::Tensor<dtype, dim>(data_ptr<dtype>(), sizes, strides);
    }
...
inline Tensor make_tensor(const std::string &dtype, const std::vector<int64_t> &sizes, char *&ptr)
{
    Tensor tensor;
    if (dtype == "bfloat16") {
