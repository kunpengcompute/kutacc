...
namespace kutacc {
void gating_attention_kernel(Tensor &input, Tensor &q, Tensor &k, Tensor &v, Tensor &gate, 
    Tensor &weighted_avg, int64_t batch, int64_t seq_len, Tensor &bias, Tensor &nonbatched_bias,
    ...
    int64_t block_size, int64_t head_size, int64_t nheads, int64_t nchannels)
{
    KUTACC_CHECK(batch > 0 && seq_len > 0 && block_size > 0 && head_size > 0 && nheads > 0 && nchannels > 0 &&
        ...
        head_size <= INT32_MAX && block_size <= INT64_MAX / seq_len, "input param <= 0 or overflow");
    ...
    addmm(kutacc::to_bf16(1),
        Tensor((__bf16 *)input.data_ptr(), {batch * seq_len, nchannels}, {nchannels, 1}, 2, kBF16),
        Tensor((__bf16 *)key_w.data_ptr(), {nchannels, nchannels}, {1, nchannels}, 2, kBF16),
        kutacc::to_bf16(0),
         Tensor((__bf16 *)k.data_ptr(), {batch * seq_len, nchannels}, {k.strides()[1], 1}, 2, kBF16),
        BlasExtendParams{.prepack_a = true, .prepack_b = true});
    addmm(kutacc::to_bf16(1),
        Tensor((__bf16 *)value_w.data_ptr(), {nchannels, nchannels}, {nchannels, 1}, 2, kBF16),
        ...
        for (int64_t _i = start; _i < end; _i++) {
            ...
            if (kv_buf_id != bi * nheads + hi) {
                gemm_pack<__bf16, __bf16>('A', 'T', 'N', (int)head_size, 0, (int)seq_len, (int)(batch * seq_len), 0,
                    (__bf16 *)v.data_ptr() + hi * v.strides()[0] + bi * v.strides()[2], v_buf.get());
            }
            addmm(kutacc::to_bf16(1), logits,
                Tensor(v_buf.get(), {seq_len, head_size}, {1, seq_len}, 2, kBF16), kutacc::to_bf16(0),
    ...
    addmm(kutacc::to_bf16(1), 
        Tensor((__bf16 *)weighted_avg.data_ptr(), {batch * seq_len, nchannels}, {weighted_avg.strides()[1], 1}, 2, kBF16),
...
{
    KUTACC_CHECK(input != nullptr && q_based_ptr != nullptr && bias != nullptr && nonbatched_bias != nullptr && weight_ptr != nullptr &&
        out != nullptr, "kutacc_af2_gating_attention: input args nullptr error");
    if (kutacc::kutacc_check_err_set == true) {
        return;
    }
    ...
    kutacc::gating_attention_kernel(*kutacc::convertKutaccTensor(input), *kutacc::convertKutaccTensor(q), *kutacc::convertKutaccTensor(k),
        *kutacc::convertKutaccTensor(v), *kutacc::convertKutaccTensor(gate), *kutacc::convertKutaccTensor(weight_avg), batch, seq_len, 
        *kutacc::convertKutaccTensor(bias), *kutacc::convertKutaccTensor(nonbatched_bias), *kutacc::convertKutaccTensor(query_w),
        *kutacc::convertKutaccTensor(key_w), *kutacc::convertKutaccTensor(value_w), *kutacc::convertKutaccTensor(gating_w), 
        *kutacc::convertKutaccTensor(gating_b), *kutacc::convertKutaccTensor(output_w), *kutacc::convertKutaccTensor(output_b),
        *kutacc::convertKutaccTensor(out), block_size, head_size, nheads, nchannels);
}